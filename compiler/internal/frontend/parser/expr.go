...
func parseUnary(p *Parser) ast.Expression {
	...
	if p.match(lexer.PLUS_PLUS_TOKEN, lexer.MINUS_MINUS_TOKEN) {
		...
		if p.match(lexer.PLUS_PLUS_TOKEN, lexer.MINUS_MINUS_TOKEN) {
			...
			p.Reports.Add(p.filePath, source.NewLocation(&operator.Start, &operator.End), errMsg).SetLevel(report.SYNTAX_ERROR)
			return nil
		}
		...
		if _, ok := operand.(*ast.PostfixExpr); ok {
			p.Reports.Add(p.filePath, source.NewLocation(&operator.Start, &operator.End), "Cannot mix prefix and postfix operators").SetLevel(report.SYNTAX_ERROR)
			return nil
		}
...
func parseIndexing(p *Parser, expr ast.Expression) (ast.Expression, bool) {
	...
	if index == nil {
		token := p.peek()
		p.Reports.Add(p.filePath, source.NewLocation(&token.Start, &token.End), report.MISSING_INDEX_EXPRESSION).SetLevel(report.SYNTAX_ERROR)
		return nil, false
	}
...
func handlePostfixOperator(p *Parser, expr ast.Expression) (ast.Expression, bool) {
	if p.match(lexer.PLUS_PLUS_TOKEN, lexer.MINUS_MINUS_TOKEN) {
		if _, ok := expr.(*ast.PrefixExpr); ok {
			current := p.peek()
			p.Reports.Add(p.filePath, source.NewLocation(&current.Start, &current.End), "Cannot mix prefix and postfix operators").SetLevel(report.SYNTAX_ERROR)
			return nil, false
		}
...
func parseFunctionCall(p *Parser, caller ast.Expression) (ast.Expression, bool) {
	...
	for !p.match(lexer.CLOSE_PAREN) {
		...
		if arg == nil {
			token := p.peek()
			p.Reports.Add(p.filePath, source.NewLocation(&token.Start, &token.End), "Expected function argument").SetLevel(report.SYNTAX_ERROR)
			return nil, false
		}
