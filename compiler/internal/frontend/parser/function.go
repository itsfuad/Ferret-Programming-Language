...
func parseParameters(p *Parser) []ast.Parameter {
	...
	for !p.match(lexer.CLOSE_PAREN) {
		...
		if !ok {
			token := p.peek()
			p.ctx.Reports.Add(p.filePath, source.NewLocation(&token.Start, &token.End), report.EXPECTED_PARAMETER_TYPE, report.PARSING_PHASE).AddHint("Add a type after the colon").SetLevel(report.SYNTAX_ERROR)
			return nil
		}
...
func parseReturnTypes(p *Parser) []ast.DataType {
	...
	if p.peek().Kind != lexer.OPEN_PAREN {
		...
		if !ok {
			token := p.previous()
			p.ctx.Reports.Add(p.filePath, source.NewLocation(&token.Start, &token.End), report.EXPECTED_RETURN_TYPE, report.PARSING_PHASE).AddHint("Add a return type after the arrow").SetLevel(report.SYNTAX_ERROR)
			return nil
		}
