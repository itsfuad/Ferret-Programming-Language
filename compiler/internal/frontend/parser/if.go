...
func parseIfStatement(p *Parser) ast.BlockConstruct {
	...
	if condition == nil {
		token := p.peek()
		p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), "Expected condition after 'if'", report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
		return nil
	}
	// Parse if body
	body := parseBlock(p)
