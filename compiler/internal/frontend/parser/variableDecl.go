...
func parseIdentifiers(p *Parser) ([]*ast.VariableToDeclare, int) {
	...
	for {
		if !p.check(lexer.IDENTIFIER_TOKEN) {
			token := p.peek()
			p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), report.MISSING_NAME, report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
			return nil, 0
		}
...
func parseTypeAnnotations(p *Parser) ([]ast.DataType, bool) {
	...
	for {
		...
		if !ok {
			token := p.peek()
			p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), report.MISSING_TYPE_NAME, report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
			return nil, false
		}
...
func parseInitializers(p *Parser) ([]ast.Expression, bool) {
	...
	if p.match(lexer.EQUALS_TOKEN) {
		...
		for {
			...
			if value == nil {
				token := p.peek()
				p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), "Expected value after '=', got invalid expression", report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
				return nil, false
			}
...
func assignTypes(p *Parser, variables []*ast.VariableToDeclare, types []ast.DataType, varCount int) bool {
	...
	token := p.peek()
	p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), report.MISMATCHED_VARIABLE_AND_TYPE_COUNT+fmt.Sprintf(": Expected %d types, got %d", varCount, len(types)), report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
	return false
}
...
func parseVarDecl(p *Parser) ast.Statement {
	...
	if variables == nil {
		pos := p.peek()
		p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&pos.Start, &pos.End), "no variables found", report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
		return nil
	}
	...
	if len(parsedTypes) == 0 {
		if len(values) == 0 {
			token := p.peek()
			p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), "cannot infer types without initializers", report.PARSING_PHASE).AddHint("👈😃 Add initializers to the variables").SetLevel(report.NORMAL_ERROR)
			return nil
		}
	}
	...
	if len(values) > varCount {
		token := p.peek()
		p.ctx.Reports.Add(p.filePathAbs, source.NewLocation(&token.Start, &token.End), "values cannot be more than the number of variables", report.PARSING_PHASE).SetLevel(report.SYNTAX_ERROR)
		return nil
	}
